...
{
   disconnectedCleanup();
   MessageBoxOK("ERRO", "Erro na conexão. Impossível conectar ao servidor do Império neste momento.");
...
{
   disconnectedCleanup();
   //MessageBoxOK("DESCONECTADO", "Login ou senha incorretos!");
   MessageBoxOK("ERRO", "Servidor desconectado: o serviço de internet está instável ou o servidor está momentaneamente fora do ar.");
   conectandoGui.setVisible(false);
}
...
{
   disconnectedCleanup();
   MessageBoxOK("DESCONECTADO", $ServerConnectionErrorMessage @ " (" @ %msg @ ")");
   conectandoGui.setVisible(false);
}
...
{
   disconnectedCleanup();
  MessageBoxOK("ERRO", "Erro na conexão. Por favor, tente novamente.");
	conectandoGui.setVisible(false);
}
